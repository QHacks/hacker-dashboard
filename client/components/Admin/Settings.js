...
const { getAdmins, getEmails, getReviewers, getSettings, getTestEmailRecipients } = selectors;
...
class Settings extends Component {
    ...
    handleTestEmailRecipientDropdownChange(e, data, { templateName }) {
        ...
        this.props.setTestEmailRecipients({ testEmailRecipients: { [templateName]: testEmailRecipients } });
    }

    handleSendTestEmailClick(templateName) {
        const { admins } = this.props;
        const recipients = this.props.testEmailRecipients[templateName].map((recipientId) => (
            admins.find((admin) => admin._id === recipientId)
        ));
                    ...
                    <Table.Body>
                        {emails.map((email) => (
                            <Table.Row key={uuid()}>
                                ...
                                <Table.Cell>{this.renderDropdownOfAdmins(email[NAME])}</Table.Cell>
                                <Table.Cell>
                                    <Button positive
    ...
    renderDropdownOfAdmins(templateName) {
        ...
        return (
            <Dropdown placeholder="Select a test email recipient"
                      fluid
                      multiple
                      selection
                      onChange={(e, data) => this.handleTestEmailRecipientDropdownChange(e, data, { templateName })}
