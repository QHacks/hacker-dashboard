...
const { SENDGRID_API_KEY = winston.warn('Missing SendGrid API key. Emails will not send.') } = process.env;
...
function createEmailsMiddleware() {
	return (req, res, next) => {
		...
		if (TEMPLATE_NAMES_BY_URL.hasOwnProperty(req.url)) res.on('finish', handleFinish);
