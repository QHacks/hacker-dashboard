...
module.exports = (ctr) => {
  ...
  adminAPI.get(`/${ADMIN}/${APPLICATIONS}/${REVIEW}`, async (req, res) => {
    ...
    try {
      ...
      const applicationToReview = await admin.getApplicationToReview(
        reviewGroup
      );
  ...
  adminAPI.post(
    `/${ADMIN}/${APPLICATIONS}/${REVIEW}/:userId`,
    async (req, res) => {
  ...
  adminAPI.put(
    `/${ADMIN}/${APPLICATIONS}/:eventId/:userId`,
    async (req, res) => {
      const { eventId, userId } = req.params;
      const { status } = req.body;
      try {
        const updatedUser = await user.updateApplicationStatus(
          userId,
          eventId,
          status
        );
  ...
  adminAPI.put(
    `/${ADMIN}/${APPLICATIONS}/:eventId/:userId/check-in`,
    async (req, res) => {
      const { eventId, userId } = req.params;
      const { status } = req.body;
      try {
