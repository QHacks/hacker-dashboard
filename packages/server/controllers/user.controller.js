...
const EMAILS_BY_APPLICATION_STATUS = {
  [USER.APPLICATION.STATUSES.ACCEPTED]:
    EMAILS.TEMPLATES.APPLICATION_ACCEPTED.NAME,
  [USER.APPLICATION.STATUSES.REJECTED]:
    EMAILS.TEMPLATES.APPLICATION_DECLINED.NAME,
  [USER.APPLICATION.STATUSES.WAITING_LIST]:
    EMAILS.TEMPLATES.APPLICATION_WAITLISTED.NAME
};
  ...
  getAllUsers() {
    return new Promise((resolve, reject) => {
      User.find()
        ...
        .catch((err) => {
          reject(
            createError(ERROR_TEMPLATES.DB_ERROR, ERROR.DB_USERS_GET, err)
          );
        });
    });
  },
