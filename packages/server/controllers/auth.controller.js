  ...
  authenticateUser(email, password) {
    return new Promise((resolve, reject) => {
      User.authenticate(email, password)
        ...
        .catch((err) => {
          reject(
            createError(ERROR_TEMPLATES.UNAUTHORIZED, ERROR.INVALID_CREDENTIALS)
          );
        });
    });
  },
  ...
  refresh(refreshToken) {
    return new Promise((resolve, reject) => {
      jwt.verify(refreshToken, AUTH_SECRET, (err, decoded) => {
        ...
        User.findOneAndUpdate({ _id: userId }, { refreshToken }, { new: true })
          .then((updatedUser) => {
            const accessToken = createAccessToken(updatedUser._id);
          ...
          .catch((err) => {
            reject(
              createError(ERROR_TEMPLATES.NOT_FOUND, ERROR.INVALID_USER_ID, err)
            );
          });
      });
    });
      ...
      customValidator
        ...
        .then((event) => {
          Hacker.create(
            ...
            .catch((err) => {
              reject(
                createError(ERROR_TEMPLATES.DB_ERROR, ERROR.DB_USER_CREATE, err)
              );
            });
        })
        .catch((err) => {
        ...
        .then((user) => {
          ...
          User.findOneAndUpdate(
            ...
            .then((updatedUser) => {
              emails
                .sendEmail(
                  EMAILS.TEMPLATES.RESET_PASSWORD.NAME,
      ...
      User.findOne({ passwordResetHash: resetHash })
        .then((user) => {
          ...
          user
            ...
            .then(() => {
              emails
                .sendEmail(EMAILS.TEMPLATES.RESET_PASSWORD_SUCCESS.NAME, user)
                .then(() => {
                  resolve();
                })
                .catch((err) => {
