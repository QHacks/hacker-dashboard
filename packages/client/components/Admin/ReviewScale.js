...
export default ({ onVote = noop, ...rest }) => {
  function renderGoldenTicketTrigger() {
    return <Button color="yellow" icon="ticket" content="Use Golden Ticket" />;
  }
  ...
  return (
    <div {...rest}>
      <div>
        <Button onClick={() => onVote(NO_SCORE)} negative>
          No
        </Button>
        <Button onClick={() => onVote(MAYBE_SCORE)}>Maybe</Button>
        <Button onClick={() => onVote(YES_SCORE)} positive>
          Yes
        </Button>
      </div>
      <div
