...
import { Button, Container, Dropdown, Header, Message, Segment } from 'semantic-ui-react';
import { connect } from 'react-redux';
import { actionCreators } from '../../HackerStore/HackerActions';
import { selectors } from '../../HackerStore/HackerReducer';
import uuid from 'uuid/v4';

const { fetchHackersRequiringCheckIn, setHackerIDToCheckIn, updateHackerCheckInStatus } = actionCreators;
const {
...
class CheckIn extends Component {
    ...
    checkInHacker(hackerId) {
        const eventId = this.props.hackers.filter((h) => h._id = hackerId)[0].events[0]; //TODO: Fix this hack shit
    ...
    renderBody() {
        const { hackers, areHackersRequiringCheckInLoading, isHackerBeingCheckedIn, selectedHackerId } = this.props;
        const isEmailDropdownLoading = areHackersRequiringCheckInLoading || isHackerBeingCheckedIn;
        ...
        return (
            <Segment>
                ...
                />
                <Button positive
                        onClick={() => this.checkInHacker(selectedHackerId)}>
...
function mapStateToProps(state, ownProps) {
    return {
        ...
        areHackersRequiringCheckInLoading: getAreHackersRequiringCheckInLoading(state),
...
export default connect(
    mapStateToProps,
    { fetchHackersRequiringCheckIn, setHackerIDToCheckIn, updateHackerCheckInStatus }
)(CheckIn);
