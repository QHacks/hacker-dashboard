...
const GRADUATION_YEARS = ['2022', '2021', '2020', '2019', '2018', 'Other'];
  ...
  return (
    <Segment className={props.className}>
      ...
      <Field
        ...
        validate={[
          required({ msg: 'Please supply your date of birth' }),
  ...
  return (
    <Segment className={props.className}>
      ...
      <Field
        ...
        validate={required({
          msg: 'Please tell us where you plan on travelling from!'
        })}
      />
    </Segment>
  );
  ...
  return (
    <Segment className={props.className}>
      <Field
        ...
        placeholder="#"
        validate={required({
          msg: 'Please select the number of hackathons that you have attended!'
        })}
        search={true}
      />
...
function SubmitButton() {
  return (
    <Button type="submit" content="Apply" primary className="button right" />
  );
}
...
class ApplyForm extends Component {
  ...
  render() {
    ...
    if (applicationPage > 0 && applicationPage < numberOfPages) {
      Buttons.push(
        <PreviousPageButton
          key="application-prev-page-button"
          onClick={(e) => {
            e.preventDefault();
    ...
    if (applicationPage >= 0 && applicationPage < numberOfPages - 1) {
      Buttons.push(
        <NextPageButton
          key="application-next-page-button"
          onClick={(e) => {
            e.preventDefault();
    ...
    return (
      ...
      >
        <FormProgress
          ...
          onNextStep={(e) =>
            this.handleNextPageUpdate(
              Number(e.currentTarget.getAttribute('data-value'))
            )
          }
          onPreviousStep={(e) =>
            this.handlePreviousPageUpdate(
              Number(e.currentTarget.getAttribute('data-value'))
            )
          }
        />
        ...
        {Pages.map((Page, index) => (
          <Page
            ...
            className={index !== applicationPage ? 'hidden' : ''}
          />
        ))}
...
ApplyForm = connect(mapStateToProps, {
  applicationFormErrorMessagesUpdate,
