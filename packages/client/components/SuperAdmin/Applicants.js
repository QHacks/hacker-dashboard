...
const REVIEW_TABLE_FETCH_DEFAULT_SETTINGS = {
...
class Applicants extends Component {
  ...
  renderViewReviewsFooter() {
    ...
    const numberOfTablePages = Math.ceil(
      applicationsWithReviewsCount / REVIEW_TABLE_APPLICATIONS_PER_PAGE
    );
    const pageButtons = [];
    ...
    pageButtons.push(
      <Menu.Item as="a" key={uuid()} active>
        {reviewTablePage + 1}
      </Menu.Item>
    );
        ...
        <Table.Row>
          <Table.HeaderCell colSpan={REVIEW_TABLE_HEADERS.length}>
            <p>
              ...
              to
              {` ${
                reviewTablePage * REVIEW_TABLE_APPLICATIONS_PER_PAGE +
                  REVIEW_TABLE_APPLICATIONS_PER_PAGE >
  ...
  render() {
    ...
    const applications = applicationsWithReviews.map((user) => {
      ...
      const application =
        applications.find((application) => application.event === event) || {};
      return {
        _id,
        event,
        firstName,
